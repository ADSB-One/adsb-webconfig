...
if (!empty($resetit)) {
	...
	if(timeleft <= 0){
		...
		window.location.replace("http://adsb.local");
	}
	...
	<p>
	<table><tr><td>
	If the unit is running on wifi, you'll need to connect to the "ADSB-config" network within 15 minutes of reboot and visit <a href="http://adsb.local">http://adsb.local</a> to reconfigure wifi.
	
	</td></tr></table>
		<p>(If you will have multiple units on your network, they will be reachable at:<br>http://adsb-2.local<br>http://adsb-3.local<br>etc....</body></html>
	
	<?php

	
	system('sudo /adsb/update/helpers/resetdefaults.sh');
