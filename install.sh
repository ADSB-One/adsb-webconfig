...
if ! [[ -f /boot/adsb-version ]]; then
    echo 8.0123456789 > /boot/adsb-version
fi
...
for file in helpers/*.sh; do
    echo "www-data ALL = NOPASSWD: /adsb/webconfig/$file" >> /etc/sudoers.d/010_www-data
done
